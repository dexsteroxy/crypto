...
const Table = () => {
  // const {text} = useContext(CryptoContext);
  let { cryptoData, currency } = useContext(CryptoContext);

  return (
  <>
    <div className=" flex flex-col mt-9 border border-blue rounded" data-aos="fade-up"
    data-aos-delay=" 400">
      {cryptoData ? (
        <table className=" w-full table-auto">
          <thead className=" capitalize text-[16px] font-medium border-b border-blue">
            <tr>
          ...
          <tbody>
            {cryptoData.map((data) => {
              return (
                <tr
                  key={data.id}
                  className=" text-center text-[16px] border-b border-blue last:border-b-0 hover:bg-darkblue-secondary"
                >
                  <td className="py-4 flex items-center uppercase">
                    <button className=" outline-0 border-0 bg-none cursor-pointer">
                      <svg
                        className="w-[1.5rem] fill-gray-100 ml-1.5 hover:fill-yellow-300"
                        width="30"
                        height="30"
                        viewBox="0 0 30 30"
                        fill="blue"
...
<Link to={`/${data.id}`} className=" cursor-pointer">
  {data.name}
    ...
    <div className=" flex items-center justify-between mt-4 capitalize h-[2rem]">
      <span>Date provided by <a href="https://www.coingecko.com" className=" text-blue" rel="noreferrer" target="blank">Amadi Sixtus</a></span>
      <Pagination/>
    </div>
